...
public static class EcsManagedData<T> {
	public static readonly ManagedListPool<(T v, short refCount)> items = new();

	public static bool Contains(Ref<T> r) => items.Contains(r.id);

	public static Ref<T> Add(T v) {
		for (int i = 0; i < items.storage.Count; i++) {
			if (items.emptyIndices.Contains(i) || !items.storage[i].Equals(v)) continue;
			Ref(i);
			return new(i);
		}
	...
	public static void Ref(int i) {
		(T v, short refCount) item = items.storage[i];
		item.refCount++;
